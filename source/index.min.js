...
function isMoving() {
    return physics.currentlyMovingBody.velocity.x >= constant.VelocityTreshold || physics.currentlyMovingBody.velocity.x <= -constant.VelocityTreshold || physics.currentlyMovingBody.velocity.y >= constant.VelocityTreshold || physics.currentlyMovingBody.velocity.y <= -constant.VelocityTreshold;
}
function move() {
    ...
    if (physics.currentlyMovingBody.velocity.y >= constant.VelocityTreshold) {
        var collisionBody = findBodyToFallOn();
    ...
    else if (physics.currentlyMovingBody.velocity.x <= -constant.VelocityTreshold && (blockingTile = getBlockingTile())) {
...
function isFallingOverlap(collisionBody) {
...
function getBlockingTile() {
    if (physics.currentlyMovingBody.velocity.y >= constant.VelocityTreshold) {
        return world.map.getTileWorldXY(physics.currentlyMovingBody.x + physics.currentlyMovingBody.velocity.x, (physics.currentlyMovingBody.y + constant.TileSize.heigth) + physics.currentlyMovingBody.velocity.y, constant.TileSize.width, constant.TileSize.heigth, "collision");
    }
    else if (physics.currentlyMovingBody.velocity.x <= -constant.VelocityTreshold) {
        return world.map.getTileWorldXY(physics.currentlyMovingBody.x + physics.currentlyMovingBody.velocity.x, physics.currentlyMovingBody.y + physics.currentlyMovingBody.velocity.y, constant.TileSize.width, constant.TileSize.heigth, "collision");
    }
    else if (physics.currentlyMovingBody.velocity.x >= constant.VelocityTreshold) {
        return world.map.getTileWorldXY((physics.currentlyMovingBody.x + constant.TileSize.width) + physics.currentlyMovingBody.velocity.x, physics.currentlyMovingBody.y + physics.currentlyMovingBody.velocity.y, constant.TileSize.width, constant.TileSize.heigth, "collision");
    }
    return undefined;
}
...
function checkCollision(target) {
    ...
    var currentX = Math.round((physics.currentlyMovingBody.x / constant.TileSize.width) + (constant.TileSize.width * 0.5));
    var currentY = Math.round((physics.currentlyMovingBody.y / constant.TileSize.heigth) + (constant.TileSize.heigth * 0.5));
    var currentLeft = currentX - 1;
    ...
    if (physics.currentlyMovingBody.velocity.x <= -constant.VelocityTreshold && currentLeft === targetX && currentY === targetY) {
        stopCurrent = true;
    }
    else if (physics.currentlyMovingBody.velocity.x >= constant.VelocityTreshold && currentRight === targetX && currentY === targetY) {
        stopCurrent = true;
    }
    if (stopCurrent) {
        if (transferForce) {
...
function transferVelocity(target) {
...
function findBodyToFallOn() {
    ...
    var tileX;
    if (!body) {
        var tileY = getTileBelow();
        var tileX = Math.floor((physics.currentlyMovingBody.x + constant.TileSize.width * 0.5) / constant.TileSize.width);
    }
    if (!tileY && !body) {
        return undefined;
    }
...
function buildCollisionBody(body) {
    return {
        tile: {
...
function getTileBelow() {
    var current = physics.currentlyMovingBody;
    var x = Math.floor((current.x + constant.TileSize.width * 0.5) / constant.TileSize.width);
    var y = Math.floor(current.y / constant.TileSize.heigth);
...
(function (physics) {
    ...
    function update(game) {
        physics.physicsBodies = physics.physicsBodies.sort(function (l, r) {
            if (l.y < r.y) {
                return 1;
            }
            else if (l.y > r.y) {
        ...
        physics.physicsBodies.forEach(function (targetBody) {
            if (!physics.currentlyMovingBody || physics.currentlyMovingBody === targetBody) {
                return;
            }
            if (checkCollision(targetBody)) {
                console.log("Collision between bodies:", physics.currentlyMovingBody.tiledType, physics.currentlyMovingBody.velocity.x, targetBody.tiledType, targetBody.velocity.x);
            }
        });
